...
Sinkhorn autoencoder reaffirm this kind of autoencoder formulation but with a stronger theoretical intuition. They show that by decomposing the Wasserstein-p loss at the LHS when the restricting $Q(Z \mid X)P(X) = P(Z)$, we will have the exact objective as mentioned by WAE, but to directly optimize this objective, we should use a specific kind of divergence for $$Q(Z)$$ and $$P(Z)$$, namely the same Wasserstein-p distance. Now we are presented with exact  the same problem
